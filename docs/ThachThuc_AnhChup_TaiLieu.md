...
| 4 | Lóa sáng dải dài dọc biên mép do đèn huỳnh quang | ✅ Đã xử lý | **Shadow Normalization** + **Soft Binarization** (Step 3c, 3d) |
| 5 | Đốm lóa trắng xóa (Glare) che khuất chữ do bật Flash | ✅ Đã xử lý | **In-painting** (Xóa đốm và nội suy vùng ảnh lân cận) (Step 3a) |
| 6 | Nhiễu hạt (ISO Noise / Salt-Pepper) do chụp thiếu sáng | ⚠️ Một phần | **Soft Binarization** (Kích trần khoảng trắng che lấp bước nhiễu râm) (Step 3d) |
| 7 | Sai lệch phổ màu trắng do môi trường ám ánh đèn vàng/đỏ | ✅ Đã xử lý | **Grayscale** kết hợp **Soft Binarization / Linear Contrast** (Step 3d) |
...
| 17 | Nền giấy cũ mốc ngả vàng, ố bẩn lốm đốm / cặn bề mặt | ✅ Đã xử lý | **Soft Binarization** (Xén ngưỡng White Point tẩy nền vô cực) (Step 3d) |
...
| 19 | Mực phai trôi, nhạt nhòa, vạch đứt đoạn không liền mạch | ✅ Đã xử lý | **Soft Binarization** (Khóa chặt Black Point chìm mực thành lõi đen sậm) (Step 3d) |
...
1. **Phân vùng (Segmentation):**
   - Sử dụng Mô hình trí tuệ nhân tạo (VD: `LayoutLM`, `YOLOv8 Document Layout`) để vẽ bounding-box khoanh vùng riêng biệt: Đâu là **Text (Chữ)**, đâu là **Figure/Picture (Hình ảnh màu)**, đâu là **Background (Nền giấy trắng)**.

2. **Xử lý Chọn lọc (Selective Processing):**
   - **Với vùng TEXT (Chữ):** Áp dụng toàn bộ Bước 3 (Khử bóng, Phơi sáng mềm Soft Binarize) để chữ đanh đen, nền vùng chữ trắng xóa.
